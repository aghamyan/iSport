...
import {
  getChampionshipInterviewStatusAction,
  getOrCreateChampionshipInterviewAction,
  sendChampionshipInterviewReplyAction,
  endChampionshipInterviewAction,
  type ChampionshipInterviewSession,
} from './championshipInterviewActions'
import type { InterviewLanguage } from '@/lib/ai/championshipInterview'

// Same "The Mic" journalist accent used in the match interview modal
...
const INTERVIEW_STYLES = `
  ...
  .ci-send-btn:not(:disabled):hover {
  ...
  @media (prefers-reduced-motion: reduce) {
    .ci-overlay, .ci-panel, .ci-bubble, .ci-pip {
...
export function ChampionshipInterviewModal({ championshipId, championshipName, playerName, onClose }: Props) {
  const [session, setSession] = useState<ChampionshipInterviewSession | null>(null)
  ...
  useEffect(() => {
    ...
    // getOrCreateChampionshipInterviewAction), this just avoids the wasted duplicate
    // call in dev.
    ...
    getChampionshipInterviewStatusAction(championshipId)
      .then((status) => {
        if (status.exists) {
          return getOrCreateChampionshipInterviewAction(championshipId)
            .then((s) => setSession(s))
  ...
  function handleChooseLanguage(language: InterviewLanguage) {
    ...
    setLoading(true)
    getOrCreateChampionshipInterviewAction(championshipId, language)
      .then((s) => setSession(s))
  ...
  function handleSend() {
    ...
    startTransition(async () => {
      try {
        const updated = await sendChampionshipInterviewReplyAction(session.id, text)
        setSession(updated)
      } catch (e) {
  ...
  function handleEnd() {
    ...
    startTransition(async () => {
      try {
        await endChampionshipInterviewAction(session.id)
  ...
  return (
    ...
    >
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Championship Interview"
      >
        {/* Header */}
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 14.5, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.01em' }}>
                The Mic <span style={{ color: 'var(--muted)', fontWeight: 600 }}>· Season-Wrap Interview</span>
              </div>
              <div style={{
                fontSize: 11.5, color: 'var(--muted)', marginTop: 1,
                overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
              }}>
                {playerName} · {championshipName}
              </div>
            </div>
          </div>
          <button
