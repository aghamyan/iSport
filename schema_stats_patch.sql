-- ============================================================
-- iSport — Stats calculation system
-- Apply AFTER: schema.sql, schema_auth.sql, schema_matches_patch.sql,
--              schema_championships_patch.sql, schema_rivalries_patch.sql
-- ============================================================


-- ──────────────────────────────────────────────────────────────
-- PERFORMANCE INDEXES
-- Partial indexes on confirmed matches for fast per-player lookups.
-- Covers recompute queries (WHERE status='confirmed' AND player_id=...)
-- and form queries (same filter + ORDER BY confirmed_at DESC).
-- ──────────────────────────────────────────────────────────────
create index idx_fm_conf_home on friendly_matches(home_player_id, confirmed_at desc)
  where status = 'confirmed';
create index idx_fm_conf_away on friendly_matches(away_player_id, confirmed_at desc)
  where status = 'confirmed';
create index idx_cm_conf_home on championship_matches(home_player_id)
  where status = 'confirmed';
create index idx_cm_conf_away on championship_matches(away_player_id)
  where status = 'confirmed';
-- For per-championship standings recompute
create index idx_cm_conf_champ_home on championship_matches(championship_id, home_player_id)
  where status = 'confirmed';
create index idx_cm_conf_champ_away on championship_matches(championship_id, away_player_id)
  where status = 'confirmed';


-- ──────────────────────────────────────────────────────────────
-- recompute_player_stats
-- Rebuilds wins/losses/draws/goals_for/goals_against for one player
-- from all confirmed friendly + championship matches.
--
-- Recompute-from-source (vs. delta ±1) is idempotent — score edits
-- within the 4-hour window fire the trigger again and produce the
-- correct total without any undo logic.
--
-- Row-level FOR UPDATE lock on the players row serialises concurrent
-- recomputes (e.g., two matches confirmed simultaneously) so the last
-- writer always sees the complete committed picture.
-- ──────────────────────────────────────────────────────────────
...
declare
  ...
  v_ga     int := 0;
begin
  -- Acquire row lock before reading source data so two concurrent
  -- recomputes for the same player are serialised, not interleaved.
  ...
  -- matches_played and goal_diff are GENERATED ALWAYS columns — do not write them.
  update players set
    wins          = v_wins,
    draws         = v_draws,
    losses        = v_losses,
...
-- ──────────────────────────────────────────────────────────────
-- recompute_championship_standings
-- Rebuilds one player's row in championship_standings for a given
-- championship. Upserts so it works for both first-time creation
-- and subsequent recalculations.
-- points and goal_diff are GENERATED columns — not written directly.
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- TRIGGER FUNCTIONS
-- Both handle INSERT (rivalry matches are inserted as 'confirmed'
-- directly) and UPDATE (normal confirm + score edits within 4h).
-- Keyed on the transition itself so a match contributes exactly once
-- (or is corrected) — no double-counting.
-- ──────────────────────────────────────────────────────────────
...
begin
  if tg_op = 'INSERT' then
    if new.status = 'confirmed' and new.home_score is not null then
      perform recompute_player_stats(new.home_player_id);
      perform recompute_player_stats(new.away_player_id);
    end if;

  elsif tg_op = 'UPDATE' then
    -- Fire on: pending→confirmed, or score change on a confirmed match
    if (old.status is distinct from new.status and new.status = 'confirmed')
...
create trigger trg_friendly_stats
  after insert or update on friendly_matches
  for each row execute function trg_friendly_stats_fn();
...
create trigger trg_championship_stats
  after insert or update on championship_matches
  for each row execute function trg_championship_stats_fn();


-- ──────────────────────────────────────────────────────────────
-- get_player_form
-- Returns the last p_limit results for a player across all match
-- types, most recent first. Includes opponent name via users join.
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- get_h2h_stats
-- Head-to-head totals between two players across all match types.
-- Caller assigns player1/player2 roles for display purposes;
-- the function does not require canonical ordering.
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- BACKFILL
-- Recomputes all existing stats in case confirmed matches existed
-- before this patch was applied. Idempotent — safe to re-run.
-- ──────────────────────────────────────────────────────────────
do $$
declare
  v_player record;
...
begin
  for v_player in select id from players loop
    perform recompute_player_stats(v_player.id);
  end loop;

  for v_pair in
    select distinct championship_id, home_player_id as player_id
      from championship_matches where status = 'confirmed'
    union
    select distinct championship_id, away_player_id
      from championship_matches where status = 'confirmed'
  loop
    perform recompute_championship_standings(v_pair.championship_id, v_pair.player_id);
  end loop;
end;
$$;
